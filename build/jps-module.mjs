...
export function parseArguments(argv) {
  ...
  if (command !== "register" && command !== "check" && command !== "unregister") {
    throw new UsageError("Command must be 'register', 'unregister', or 'check'.")
  }
  if (command === "unregister" && fixImlEof) {
    throw new UsageError("The unregister command does not change .iml files.")
  }
...
function resolveImlPath(rawPath, runtime, allowMissing = false) {
  ...
  if (!allowMissing && !runtime.exists(absolutePath)) {
...
export function updateModulesXmlContent(text, projectHome, modulesXmlPath, modulePaths, options = {}) {
  ...
  const removeEntries = options.removeEntries ?? false
  const requestedFilepaths = new Set(modulePaths.map((path) => createModuleFilePath(projectHome, path)))
  const diagnostics = []
  const entryByFilepath = new Map()
  const keptEntries = []

  for (const entry of parsed.entries) {
    if (removeEntries && requestedFilepaths.has(entry.filepath)) {
  ...
  for (const modulePath of removeEntries ? [] : modulePaths) {
...
export function runOperation(args, runtime = createDefaultRuntime(), io = createDefaultIo()) {
  const removeEntries = args.command === "unregister"
  const imlPaths = args.imlPaths.map((imlPath) => resolveImlPath(imlPath, runtime, removeEntries))
  const write = args.command === "register" || removeEntries
  const changedFiles = []
  ...
  for (const update of createProjectUpdates(imlPaths, runtime)) {
    ...
    const result = updateModulesXmlContent(
      ...
      update.modulePaths,
      {dropEntriesOutsideProjectHome: update.dropEntriesOutsideProjectHome, removeEntries},
    )
    if (!result.changed) {
      continue
    }
