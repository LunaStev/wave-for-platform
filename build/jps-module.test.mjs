...
describe("jps-module removal", () => {
  it("removes a deleted module from both project structures and preserves other entries", async () => {
    const root = await createFixtureRoot()
    try {
      const rootXml = join(root, ".idea/modules.xml")
      const communityXml = join(root, "community/.idea/modules.xml")
      await writeTextFile(rootXml, createModulesXml([
        "$PROJECT_DIR$/community/python/intellij.python.iml",
        "$PROJECT_DIR$/community/platform/intellij.platform.iml",
      ]))
      await writeTextFile(communityXml, createModulesXml([
        "$PROJECT_DIR$/python/intellij.python.iml",
        "$PROJECT_DIR$/platform/intellij.platform.iml",
      ]))
      const {runtime, writes} = createRuntime(root)
      const {io} = createRecordingIo()
      equal(await runCli(["unregister", "community/python/intellij.python.iml"], {runtime, io}), 0)
      deepEqual(moduleLines(await readFile(rootXml, "utf8")), moduleLines(createModulesXml([
        "$PROJECT_DIR$/community/platform/intellij.platform.iml",
      ])))
      deepEqual(moduleLines(await readFile(communityXml, "utf8")), moduleLines(createModulesXml([
        "$PROJECT_DIR$/platform/intellij.platform.iml",
      ])))
...
describe("jps-module argument parsing", () => {
  it("parses unregister arguments", () => {
    deepEqual(parseArguments(["unregister", "python/intellij.python.iml"]), {
      help: false,
      command: "unregister",
      imlPaths: ["python/intellij.python.iml"],
      fixImlEof: false,
    })
  })
