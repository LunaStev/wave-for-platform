...
internal class BazelBuildFileGenerator(
  ...
  private fun BuildFile.generateBuildTargets(moduleDescriptor: ModuleDescriptor, moduleList: ModuleList): ModuleTargets {
    ...
    if (mustGenerateFleetPluginServicesResources) {
      ...
      load("@community//platform-shared/build:fleet.bzl", ruleName)
      target(ruleName) {
    ...
    target("jvm_library") {
      ...
      if (module.name == "fleet.util.multiplatform" || module.name == "intellij.platform.multiplatformSupport") {
        option("exported_compiler_plugins", listOf("@community//platform-shared/compiler-plugins/expects:expects-plugin"))
      }
      else if (module.name == "fleet.rpc") {
        option("exported_compiler_plugins", listOf("@community//platform-shared/compiler-plugins/rpc:rpc-plugin"))
      }
      else if (module.name == "fleet.noria.cells") {
        option("exported_compiler_plugins", listOf("@community//platform-shared/compiler-plugins/noria:noria-plugin"))
      }
